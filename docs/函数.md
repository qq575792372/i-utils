...
function debounce<T>(
   fn: T, 
   delay: number, 
   immediate: boolean): (this: ThisParameterType<T>, ...args: Parameters<T>) => void;
```
...
```ts
function throttle<T>(fn: T, interval: number): (this: ThisParameterType<T>, ...args: Parameters<T>) => void;
```
